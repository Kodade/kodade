...
- The tabbed workspace shell — Agents, Code, Task, and Editor tabs with a
  persistent workspace sidebar; tabs keep running in the background. The
  classic 1.x layout remains available as an escape hatch for one release.
- Agent personas and runs in the Agents tab, prepared onto KödWork and viewed
  on their own dedicated Task tab, with MCP Connections from a curated
  catalog or custom entries installed into the CLI's own config through
  reviewed changes — strictly bring-your-own-key
