-- Independent historical v5 fixture with linked memories, a checkpoint,
-- activity, and audit rows before the v6 audit session column.
...
INSERT INTO checkpoint_fts(checkpoint_id, workspace_id, summary)
SELECT id, workspace_id, summary FROM checkpoints;
INSERT INTO activity_events(
    id, workspace_id, kind, source, session_id, relative_path, provider, occurred_at
) VALUES
    (
        'act_legacy_start', 'ws_legacy', 'session_started', 'legacy-ui',
        'legacy-session', NULL, 'codex', 1700000005000
    ),
    (
        'act_legacy_save', 'ws_legacy', 'file_saved', 'legacy-ui',
        'legacy-session', 'src-tauri/src/memory.rs', 'codex', 1700000006000
    );
INSERT INTO mcp_audit(
    id, workspace_id, client, capability, action, target_id, result, occurred_at
) VALUES (
    'audit_legacy_remember', 'ws_legacy', 'legacy-mcp', 'memory:write',
    'remember', 'mem_legacy_decision', 'ok', 1700000007000
);
